...
// pci_vendor_id and pci_device_id values below are Amazon's and
// avaliable to use for a given FPGA slot. Users may replace these with
// their own if allocated to them by PCI SIG.
...
int main(int argc, char **argv)
{
  ...
  rc = peek_poke_example( value, 0, FPGA_APP_PF, APP_PF_BAR0 );
...
int peek_poke_example( uint32_t value, int slot_id, int pf_id, int bar_id )
{
  ...
  rc = fpga_pci_poke( pci_bar_handle, 0x00000000, 0xdeadbeef );
  fail_on( rc, out, "Unable to write to the FPGA!" );

  /* read it back and print it out; you should expect the byte order to be
   * reversed (That's what this CL does) */

  printf("Reading from addr 0\n");

  uint32_t ret_value;
  rc = fpga_pci_peek( pci_bar_handle, 0x00000000, &ret_value );
